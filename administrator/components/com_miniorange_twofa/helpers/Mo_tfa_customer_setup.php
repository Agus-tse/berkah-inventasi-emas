...
class Mo_tfa_Customer{
    function getCustomerKeys($email,$password) {
        if(!Mo_tfa_utilities::is_curl_installed()) {
            return json_encode(array("apiKey"=>'CURL_ERROR','token'=>JText::_("COM_MINIORANGE_SETUP2FA_CURL_ERROR1")));
        }
    ...
    {
        if(!Mo_tfa_utilities::is_curl_installed()) {
            return json_encode(array("statusCode"=>'ERROR','error'=>JText::_("COM_MINIORANGE_SETUP2FA_CURL_ERROR1")));
        }

        $url =  commonUtilitiesTfa::getApiUrls()['feedback'];
        
        $subject            = "MiniOrange Joomla 2FA Feedback";
    ...
    function submit_contact_us( $q_email, $q_phone, $query ) {
        if(!Mo_tfa_utilities::is_curl_installed()) {
            return json_encode(array("status"=>'CURL_ERROR','statusMessage'=>JText::_("COM_MINIORANGE_SETUP2FA_CURL_ERROR1")));
        }
        $hostname        = Mo_tfa_utilities::getHostname();
    ...
    function submit_trial_request($demo, $email,$mobile_number,$plan, $description){
        ...
        $content='<div>Hello, 
                  ...
                  <strong><br><br>Mobile number:</strong>'.$mobile_number.'
                  <strong><br><br>Plugin :</strong> ' .$plan. '
